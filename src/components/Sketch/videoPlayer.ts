...
export class VideoPlayer {
    ...
    private inputMovieHeight: any;
    private reScaledMovieWidth: any;
    private reScaledMovieHeight: any;
    private movieDiv: any;
    private sketch: any;
    ...
    constructor(fileLocation: string, sketch: Sketch) {
        ...
        this.sketch = sketch.getP5Sketch();

        this.movieDiv = this.sketch.createVideo(fileLocation, () => {
            ...
            this.setDisplayMovieSize(this.sketch.videoContainer.width, this.sketch.videoContainer.height);
            this.movieDiv.onload = () => URL.revokeObjectURL(fileLocation);

            const elem = document.getElementById('moviePlayer');
            if (elem) {
                elem.onended = () => this.sketch.mediator.setIsRecording(false); // end program recording when movie ends
            }

            this.sketch.mediator.newVideoLoaded();
        });
    }
     ...
     */
    setDisplayMovieSize(containerWidth: number, containerHeight: number) {
